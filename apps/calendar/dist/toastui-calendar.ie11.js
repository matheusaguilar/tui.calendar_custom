...
function getTimeSteps(timeGridRows) {
  var hourStart = Number(timeGridRows[0].startTime.split(':', 1));
  var hourEnd = Number(timeGridRows[timeGridRows.length - 1].endTime.split(':', 1));
  var STEPS = timeGridRows.length / (hourEnd - hourStart);
  var STEP_MINUTES = 60 / STEPS;
  return {
    STEPS: STEPS,
...
function createTimeGridDataRows(options) {
  ...
  var steps = (options.hourEnd - options.hourStart) * timeStep.length;
  var baseHeight = 100 / steps;
  var count = 0;
  var rows = range_default()(steps).map(function (step, index) {
    count = count === timeStep.length ? 0 : count;
    var hour = options.hourStart + Math.floor(step / timeStep.length);
    var startTime = "".concat(hour, ":").concat(timeStep[count] !== 0 ? timeStep[count] : '00').padStart(5, '0');
    var endTime = (count === timeStep.length - 1 ? "".concat(hour + 1, ":00") : "".concat(hour, ":").concat(timeStep[count + 1])).padStart(5, '0');
    count += 1;
    return {
...
function createTimeGridData(datesOfWeek, options) {
  ...
  var rows = createTimeGridDataRows(options);
  return {
    columns: columns,
    rows: rows
  };
}
function createDayHorizontalViewTimeGridData(calendarIds, datesOfWeek, options) {
  var columns = calendarIds.map(function (calendarId, index) {
    var columnWidth = 100 / calendarIds.length;
    return {
...
function useDrag(draggingItemType) {
  ...
  var store = useInternalStore();
  var currentView = store.getState().view.currentView;
  var horizontalDayView = store.getState().options.week.horizontalDayView;
  ...
  var shouldPreventHorizontalMove = F(function () {
  ...
  var handleMouseDown = hooks_module_T(function (e) {
    ...
    e.preventDefault();
    setStarted(true);
    var columnElementProps = (_e$currentTarget = e.currentTarget) === null || _e$currentTarget === void 0 ? void 0 : (_e$currentTarget$pare = _e$currentTarget.parentElement) === null || _e$currentTarget$pare === void 0 ? void 0 : _e$currentTarget$pare.getBoundingClientRect();
    initDrag({
      draggingItemType: draggingItemType,
      initX: shouldPreventHorizontalMove && columnElementProps ? columnElementProps.x + columnElementProps.width : e.clientX,
      initY: e.clientY
    });
...
function useTimeGridEventResize(_ref) {
  ...
  useWhen(function () {
    ...
    if (shouldUpdate) {
      ...
      var _getTimeSteps = getTimeSteps(timeGridData.rows),
          STEP_MINUTES = _getTimeSteps.STEP_MINUTES;

      var minEndDate = addMinutes(resizingStartUIModel.getStarts(), STEP_MINUTES);
      eventBus.fire('beforeUpdateEvent', {
...
var GridLines = compat_module_g(function GridLines(_ref) {
  ...
  var _getTimeSteps = getTimeSteps(timeGridRows),
      STEPS = _getTimeSteps.STEPS;
  ...
  }, timeGridRows.map(function (time) {
    count = count === STEPS + 1 ? 1 : count;
    var isUpperLine = count !== STEPS;
    count += 1;
    return h("div", {
...
function getMovingEventPosition(_ref) {
  ...
  var _getTimeSteps = getTimeSteps(timeGridDataRows),
      STEP_MINUTES = _getTimeSteps.STEP_MINUTES,
      MS_PER_STEP_MINUTES = _getTimeSteps.MS_PER_STEP_MINUTES;

  var rowHeight = timeGridDataRows[0].height;
  var maxHeight = rowHeight * timeGridDataRows.length;
  var millisecondsDiff = rowDiff * MS_PER_STEP_MINUTES + columnDiff * MS_PER_DAY;
  ...
  var startIndex = Math.max(getCurrentIndexByTime(nextStart, hourStart, STEP_MINUTES), 0);
  var endIndex = Math.min(getCurrentIndexByTime(nextEnd, hourStart, STEP_MINUTES), timeGridDataRows.length - 1);
...
function useTimeGridEventMove(_ref2) {
  ...
  var _getTimeSteps2 = getTimeSteps(timeGridData.rows),
      MS_PER_STEP_MINUTES = _getTimeSteps2.MS_PER_STEP_MINUTES;
  ...
  var nextStartTime = F(function () {
    ...
    return addMilliseconds(startDateTime, gridDiff.rowDiff * MS_PER_STEP_MINUTES + gridDiff.columnDiff * MS_PER_DAY);
...
var TimeColumn = compat_module_g(function TimeColumn(_ref3) {
  ...
  var rowsByHour = F(function () {
    return timeGridRows.filter(function (_, index) {
      return index % STEPS === 0 || index === timeGridRows.length - 1;
    });
  }, [timeGridRows, STEPS]);
  ...
  var totalUIModels = F(function () {
    return columns.map(function (_ref2) {
      var calendarId = _ref2.calendarId,
          date = _ref2.date;
      return events.filter(calendarId ? function (event) {
        return event.model.calendarId === calendarId;
      } : column_isBetween(toStartOfDay(date), toEndOfDay(date))) // NOTE: prevent shared reference between columns
...
function getActivePanels(taskView, eventView, horizontalDayView) {
  var activePanels = [];
...
;// CONCATENATED MODULE: ./src/components/dayGridWeek/horizontalGridRow.tsx
...
function day_Day() {
  ...
  var narrowWeekend = weekOptions.narrowWeekend,
      ...
      timeStep = weekOptions.timeStep,
      horizontalDayView = weekOptions.horizontalDayView;
  var days = F(function () {
    return [renderDate];
  }, [renderDate]);
  ...
  var calendarIds = F(function () {
    return calendar.calendars.map(function (calendarMap) {
      return calendarMap.id;
    });
  }, [calendar.calendars]);
  var showHorizontalView = F(function () {
    return calendarIds.length > 0 && horizontalDayView;
  }, [calendarIds, horizontalDayView]);
  var timeGridData = F(function () {
    return showHorizontalView ? createDayHorizontalViewTimeGridData(calendarIds, days, {
      hourStart: hourStart,
      hourEnd: hourEnd,
      narrowWeekend: narrowWeekend,
      timeStep: timeStep
