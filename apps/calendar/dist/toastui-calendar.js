...
function createTimeGridDataRows(options) {
  ...
  const rows = range_default()(steps).map((step, index) => {
    ...
    const startTime = "".concat(hour, ":").concat(timeStep[count] !== 0 ? timeStep[count] : '00').padStart(5, '0');
    const endTime = (count === timeStep.length - 1 ? "".concat(hour + 1, ":00") : "".concat(hour, ":").concat(timeStep[count + 1])).padStart(5, '0');
    count += 1;
    return {
...
function createDayHorizontalViewTimeGridData(calendarIds, datesOfWeek, options) {
  const columns = calendarIds.map((calendarId, index) => {
...
function useDrag(draggingItemType) {
  ...
  const shouldPreventHorizontalMove = F(() => {
  ...
  const handleMouseDown = hooks_module_T(e => {
    ...
    e.preventDefault();
    setStarted(true);
    const columnElementProps = (_e$currentTarget = e.currentTarget) === null || _e$currentTarget === void 0 ? void 0 : (_e$currentTarget$pare = _e$currentTarget.parentElement) === null || _e$currentTarget$pare === void 0 ? void 0 : _e$currentTarget$pare.getBoundingClientRect();
    initDrag({
      draggingItemType,
...
function useTimeGridEventResize(_ref) {
  ...
  useWhen(() => {
    ...
    if (shouldUpdate) {
      ...
      const {
        STEP_MINUTES
      } = getTimeSteps(timeGridData.rows);
...
function getMovingEventPosition(_ref) {
  ...
  const endIndex = Math.min(getCurrentIndexByTime(nextEnd, hourStart, STEP_MINUTES), timeGridDataRows.length - 1);
...
const TimeColumn = compat_module_g(function TimeColumn(_ref3) {
  ...
  const rowsByHour = F(() => timeGridRows.filter((_, index) => index % STEPS === 0 || index === timeGridRows.length - 1), [timeGridRows, STEPS]);
...
function TimeGrid(_ref) {
  ...
  const totalUIModels = F(() => columns.map(_ref2 => {
    ...
    return events.filter(calendarId ? event => event.model.calendarId === calendarId : column_isBetween(toStartOfDay(date), toEndOfDay(date))) // NOTE: prevent shared reference between columns
...
function HorizontalGridRow(_ref) {
  ...
  const panelRef = hooks_module_s(null);
  const [panelWidth, setPanelWidth] = hooks_module_y(150);
  hooks_module_(() => {
