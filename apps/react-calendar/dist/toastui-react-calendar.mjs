...
function createTimeGridDataRows(options) {
  ...
  const rows = range_1(steps).map((step, index) => {
    ...
    const startTime = `${hour}:${timeStep[count] !== 0 ? timeStep[count] : "00"}`.padStart(5, "0");
...
function useDrag(draggingItemType, { onInit, onDragStart, onDrag, onMouseUp, onPressESCKey } = {}) {
  ...
  const shouldPreventHorizontalMove = F$2(() => {
  ...
  const handleMouseDown = T$1((e2) => {
    ...
    e2.preventDefault();
    setStarted(true);
    const columnElementProps = (_b = (_a = e2.currentTarget) == null ? void 0 : _a.parentElement) == null ? void 0 : _b.getBoundingClientRect();
    initDrag({
      draggingItemType,
      initX: shouldPreventHorizontalMove && columnElementProps ? columnElementProps.x + columnElementProps.width : e2.clientX,
      initY: e2.clientY
    });
  ...
  const handleMouseMove = T$1((e2) => {
    ...
    setDragging({
      x: shouldPreventHorizontalMove ? initX : e2.clientX,
      y: e2.clientY
    });
...
const TimeColumn = g$1(function TimeColumn2({ timeGridRows, nowIndicatorState }) {
  ...
  const rowsByHour = F$2(() => timeGridRows.filter((_2, index) => index % STEPS === 0 || index === timeGridRows.length - 1), [timeGridRows, STEPS]);
...
function HorizontalGridRow({ calendars, timeGridData }) {
  ...
  }, calendars.map((calendar) => /* @__PURE__ */ h$3("div", {
    key: calendar.id,
    style: {
      display: "flex",
      width: `100%`,
...
function Day$1() {
  ...
  const activePanels = F$2(() => getActivePanels(taskView, eventView, showHorizontalView), [taskView, eventView, showHorizontalView]);
  const gridRows = F$2(() => activePanels.map((key) => {
    var _a2, _b2;
    if (key === "time") {
      return null;
    }
...
var _jsxFileName = "/home/maguila/Documents/Estudos/teste_micro_apps/tui.calendar_custom-main/apps/react-calendar/src/index.tsx";
